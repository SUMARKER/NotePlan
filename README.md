...
- **日历视图**：主界面可在 笔记 / 周计划 / 月历 / 年 之间切换。
  - **周计划**：一周七列总览本周排期任务，复选框直接勾选完成（自动写回源文件，
    ...
    "本周目标"卡片自动保存到 `Notes/周计划/年-W周.md`；顶部显示本周任务完成率。
  - **月历**：每格显示日历事件、任务时间块、待办与"有笔记"标记；拖入任务即改期；
    双击某天打开每日笔记。
  - **时间轴**：月历右侧显示选中日 06:00–23:00 的时间轴，时间块任务与事件按时间定位；
    把任务拖到时间轴某位置即设置起止时间（写入源文件 `HH:MM-HH:MM`）。
  - **年视图**：12 个迷你月历，蓝点=有笔记、紫点=有事件，点月份进入月视图。
  ...
  - **法定节假日**：月历 / 年视图 / 周计划 / 周条 / 侧栏日历中，红色日期为中国大陆法定
    节假日（休），蓝色「班」角标为调休上班日，时间轴标题也会标注当天节日；内置
  ...
  - **农历与二十四节气**：月历每格日期下方显示农历日（初一显示月名）、节气（绿色）
    与传统农历节日（春节 / 元宵 / 端午 / 中秋 / 重阳 / 腊八 / 除夕等，红色），时间轴与
    悬浮提示显示完整干支年 + 生肖 + 农历日期；算法覆盖 1900–2100 年。
...
- **任务管理**：
  ...
  - 时间块：`- [ ] 任务 >2026-09-05 14:00-15:30`，出现在月历与时间轴上；
